...
texture_model.add(Convolution2D(32, 5, 5, border_mode = 'same', input_shape = (1, opts["img_patch_size"], opts["img_patch_size"])))
texture_model.add(Convolution2D(48, 5, 5, border_mode = 'same'))
texture_model.add(MaxPooling2D(pool_size = (3, 3), stride = (2, 2)))
texture_model.add(Convolution2D(64, 5, 5, border_mode = 'same'))
texture_model.add(Convolution2D(64, 5, 5, border_mode = 'same'))
texture_model.add(MaxPooling2D(pool_size = (3, 3), stride = ([2, 2])))
texture_model.add(Convolution2D(64, 5, 5, border_mode = 'same'))
...
#texture_model.add(Activation('relu'))
#texture_model.add(MaxPooling2D(pool_size=(2, 2)))
#texture_model.add(Convolution2D(128, 3, 3, border_mode= 'valid', W_regularizer = l2(0.01), b_regularizer = l2(0.01)))
#texture_model.add(Activation('relu'))
#texture_model.add(MaxPooling2D(pool_size=(2, 2)))
#texture_model.add(Flatten())
...
# y = test_x.reshape(256, 256)
# uv = test_y.reshape(2, 256, 256)
# u = uv[0, :, :]
# v = uv[1, :, :]
#
# [r, g, b] = yuv2rgb(y, u, v)
